              ...
              <button
                onClick={() => {
                  ...
                  const getFileContent = (exampleNum: number) => {
                    if (exampleNum === 1) {
                      return `기본 템플릿
3문단 구조 글로 이뤄져있고, 각 문단마다 의견→이유→근거 패턴을 반복하되, 문단 간 키워드로 유기적 연결성을 확보해야 한다. 문단1의 "LED조명" → 문단2 재언급, 문단2의 "의사의 정교함" → 문단3 연결처럼 핵심어를 계승하며, 전체적으로는 일반론에서 구체적 메커니즘,점진적 심화를 통해 논리적 완결성을 달성하는 구조다.
